...
const Playlists = () => {
  ...
  const {playlists,getTracks,isLoading,trackList} = useContext(PlaylistsContext);
...
const trackslistRequest = async () => {
   const list =  await getTracks(currentPlaylist);
    isLoading === false && currentPlaylist && list?.length === 0? setEmpty('Цей плейлист пустий'): setEmpty('')
  }
  trackslistRequest()
  },[searchParams])
  ...
  return (
    <PageWrapper>
      ...
      currentPlaylist ? (trackList[currentPlaylist]?.length > 0 && currentPlaylist ? <Tracklist tracks={trackList[currentPlaylist]} />: <div>{empty} </div>) :playlists?.map((item) => (
        <div
          id={item.id.toString()}
