...
const Playlists = () => {
  ...
  const notify = () =>
    toast("Playlist has been deleted", {
      duration: 1500,
      style: {},
    });
  ...
  useEffect(() => {
    const handleClickedPlaylist = (e: Event) => {
      ...
      target.className.includes("deleteIcon") && +target.id == clickedPlaylist
  ...
  const getClickedTracks = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const playlistId = target.id;
    if (
      !target.className.includes("optionContainer") &&
      !target.className.includes("isDelete") &&
        ...
        <div className={classes.wrapper}>
          ...
          <div className={classes.carousel} ref={ref}>
            {playlists?.map((item, index) => (
              <div
                onClick={getClickedTracks}
                key={item.id}
                className={classes.playlistsWrapper}
              >
                <div
                ...
                >
                  {item.title}
                  {clickedDelete && clickedPlaylist === +item.id ? (
                    <div
                      id={item.id.toString()}
                      className={classes.isDeletePlaylist}
